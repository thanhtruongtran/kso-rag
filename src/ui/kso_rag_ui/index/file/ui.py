...
from kso_rag_ui.utils.render import Render
...
class FileIndexPage(BasePage):
    ...
    def download_single_file(self, is_zipped_state, file_id):
        ...
        zip_files = []
        for file_name in os.listdir(flowsettings.KSO_RAG_CHUNKS_OUTPUT_DIR):
            if target_file_name.stem in file_name:
                zip_files.append(
                    os.path.join(flowsettings.KSO_RAG_CHUNKS_OUTPUT_DIR, file_name)
                )
        for file_name in os.listdir(flowsettings.KSO_RAG_MARKDOWN_OUTPUT_DIR):
            if target_file_name.stem in file_name:
                zip_files.append(
                    os.path.join(flowsettings.KSO_RAG_MARKDOWN_OUTPUT_DIR, file_name)
                )
        zip_file_path = os.path.join(
            flowsettings.KSO_RAG_ZIP_OUTPUT_DIR, target_file_name.stem
        )
    ...
    def download_single_file_simple(self, is_zipped_state, file_html, file_id):
        ...
        output_file_path = os.path.join(
            flowsettings.KSO_RAG_ZIP_OUTPUT_DIR, target_file_name.stem + ".html"
        )
    ...
    def download_all_files(self):
        ...
        zip_files = []
        for file_name in os.listdir(flowsettings.KSO_RAG_CHUNKS_OUTPUT_DIR):
            zip_files.append(os.path.join(flowsettings.KSO_RAG_CHUNKS_OUTPUT_DIR, file_name))
        for file_name in os.listdir(flowsettings.KSO_RAG_MARKDOWN_OUTPUT_DIR):
            zip_files.append(
                os.path.join(flowsettings.KSO_RAG_MARKDOWN_OUTPUT_DIR, file_name)
            )
        zip_file_path = os.path.join(flowsettings.KSO_RAG_ZIP_OUTPUT_DIR, "all")
        with zipfile.ZipFile(f"{zip_file_path}.zip", "w") as zipMe:
            for file in zip_files:
                arcname = Path(file)
    ...
    def index_fn(
        ...
        if urls:
            ...
            files, unzip_errors = self._may_extract_zip(
                files, flowsettings.KSO_RAG_ZIP_INPUT_DIR
            )
