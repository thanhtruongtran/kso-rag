...
from kso_rag_ui import extension_protocol
from kso_rag_ui.assets import PDFJS_PREBUILT_DIR, KsoRagTheme
from kso_rag_ui.components import reasonings
from kso_rag_ui.exceptions import HookAlreadyDeclared, HookNotDeclared
from kso_rag_ui.index import IndexManager
from kso_rag_ui.settings import BaseSettingGroup, SettingGroup, SettingReasoningGroup

BASE_PATH = os.environ.get("GR_FILE_ROOT_PATH", "")


class BaseApp:
    """The main app of kso-rag
    ...
    def __init__(self):
        self.dev_mode = getattr(settings, "KSO_RAG_MODE", "") == "dev"
        self.app_name = getattr(settings, "KSO_RAG_APP_NAME", "kso-rag")
        self.app_version = getattr(settings, "KSO_RAG_APP_VERSION", "")
        self.f_user_management = getattr(
            settings, "KSO_RAG_FEATURE_USER_MANAGEMENT", False
        )
        self._theme = KsoRagTheme()
        ...
        with (dir_assets / "js" / "main.js").open() as fi:
            self._js = fi.read()
            self._js = self._js.replace("KSO_RAG_APP_VERSION", self.app_version)
    ...
    def register_reasonings(self):
        ...
        if getattr(settings, "KSO_RAG_REASONINGS", None) is None:
            return

        for value in settings.KSO_RAG_REASONINGS:
    ...
    def register_extensions(self):
        ...
        self.exman = pluggy.PluginManager("kso_rag_ui")
        self.exman.add_hookspecs(extension_protocol)
        self.exman.load_setuptools_entrypoints("kso_rag_ui")

        # retrieve and register extension declarations
        extension_declarations = self.exman.hook.kso_rag_ui_declare_extensions()
