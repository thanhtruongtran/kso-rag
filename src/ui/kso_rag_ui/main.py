...
KSO_RAG_ENABLE_FIRST_SETUP = getattr(flowsettings, "KSO_RAG_ENABLE_FIRST_SETUP", False)
KSO_RAG_APP_DATA_EXISTS = getattr(flowsettings, "KSO_RAG_APP_DATA_EXISTS", True)

# override first setup setting
if config("KSO_RAG_FIRST_SETUP", default=False, cast=bool):
    KSO_RAG_APP_DATA_EXISTS = False


def toggle_first_setup_visibility():
    global KSO_RAG_APP_DATA_EXISTS
    is_first_setup = not KSO_RAG_DEMO_MODE and not KSO_RAG_APP_DATA_EXISTS
    KSO_RAG_APP_DATA_EXISTS = True
...
class App(BaseApp):
    ...
    def ui(self):
        ...
        with gr.Tabs() as self.tabs:
            if self.f_user_management:
                from kso_rag_ui.pages.login import LoginPage
            ...
            if len(self.index_manager.indices) == 1:
                for index in self.index_manager.indices:
                    with gr.Tab(
                        ...
                        id="indices-tab",
                        visible=not self.f_user_management and not KSO_RAG_DEMO_MODE,
    ...
    def on_subscribe_public_events(self):
        if self.f_user_management:
            ...
            from kso_rag_ui.db.models import User
