...
from kso_rag_core.embeddings.base import BaseEmbeddings
...
class EmbeddingManager:
    ...
    def __init__(self):
        ...
        if hasattr(flowsettings, "KSO_RAG_EMBEDDINGS"):
            with Session(engine) as sess:
                count = sess.query(EmbeddingTable).count()
            if not count:
                for name, model in flowsettings.KSO_RAG_EMBEDDINGS.items():
                    self.add(
                        name=name,
                        spec=model["spec"],
    ...
    def load_vendors(self):
        from kso_rag_core.embeddings import (
