...
from kso_rag_core.indices.ingests.files import KSO_RAG_DEFAULT_FILE_EXTRACTORS
from kso_rag_core.indices.qa.utils import strip_think_tag
from kso_rag_ui.app import BasePage
from kso_rag_ui.components import reasonings
from kso_rag_ui.db.models import Conversation, engine
from kso_rag_ui.index.file.ui import File
from kso_rag_ui.reasoning.prompt_optimization.mindmap import MINDMAP_HTML_EXPORT_TEMPLATE
from kso_rag_ui.reasoning.prompt_optimization.suggest_conversation_name import (
    SuggestConvNamePipeline,
)
from kso_rag_ui.reasoning.prompt_optimization.suggest_followup_chat import (
...
KSO_RAG_WEB_SEARCH_BACKEND = getattr(flowsettings, "KSO_RAG_WEB_SEARCH_BACKEND", None)
WebSearch = None
if KSO_RAG_WEB_SEARCH_BACKEND:
    try:
        WebSearch = import_dotted_string(KSO_RAG_WEB_SEARCH_BACKEND, safe=False)
    except (ImportError, AttributeError) as e:
        print(f"Error importing {KSO_RAG_WEB_SEARCH_BACKEND}: {e}")

REASONING_LIMITS = 2 if KSO_RAG_DEMO_MODE else 10
...
class ChatPage(BasePage):
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1, elem_id="conv-settings-panel") as self.conv_column:
                ...
                for index_id, index in enumerate(self._app.index_manager.indices):
                    ...
                    if KSO_RAG_DEMO_MODE and is_first_index:
            ...
            with gr.Column(scale=6, elem_id="chat-area"):
                ...
                with gr.Accordion(
                    ...
                    with gr.Row(elem_id="quick-setting-labels"):
                        ...
                        gr.HTML(
                            "Model", visible=not KSO_RAG_DEMO_MODE and not KSO_RAG_SSO_ENABLED
                        )
                        gr.HTML("Language")
    ...
    def on_register_events(self):
        ...
        if KSO_RAG_DEMO_MODE and len(self._indices_input) > 0:
    ...
    def chat_fn(
        ...
        *selecteds,
        ...
        msg_placeholder = getattr(
            flowsettings, "KSO_RAG_CHAT_MSG_PLACEHOLDER", "Thinking ..."
        )
        ...
        if not text:
            empty_msg = getattr(
                flowsettings, "KSO_RAG_CHAT_EMPTY_MSG_PLACEHOLDER", "(Sorry, I don't know)"
            )
