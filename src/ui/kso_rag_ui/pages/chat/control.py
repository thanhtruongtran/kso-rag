...
from kso_rag_ui.db.models import Conversation, User, engine
...
class ConversationControl(BasePage):
    ...
    def load_chat_history(self, user_id):
        ...
        with Session(engine) as session:
            ...
            if result is not None:
                if flowsettings.KSO_RAG_USER_CAN_SEE_PUBLIC:
                    can_see_public = (
                        result.username == flowsettings.KSO_RAG_USER_CAN_SEE_PUBLIC
                    )
                else:
                    can_see_public = True
    ...
    def rename_conv(self, conversation_id, new_name, is_renamed, user_id):
        ...
        if not is_renamed or KSO_RAG_DEMO_MODE or user_id is None or not conversation_id:
            return (
                gr.update(),
                conversation_id,
