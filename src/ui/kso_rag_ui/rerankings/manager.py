...
from kso_rag_core.rerankings.base import BaseReranking
...
class RerankingManager:
    ...
    def __init__(self):
        ...
        if hasattr(flowsettings, "KSO_RAG_RERANKINGS"):
            with Session(engine) as sess:
                count = sess.query(RerankingTable).count()
            if not count:
                for name, model in flowsettings.KSO_RAG_RERANKINGS.items():
                    self.add(
                        name=name,
                        spec=model["spec"],
    ...
    def load_vendors(self):
        from kso_rag_core.rerankings import (
            CohereReranking,
            TeiFastReranking,
            VoyageAIReranking,
        )
