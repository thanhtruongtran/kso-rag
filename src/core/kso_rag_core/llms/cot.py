...
class Thought(BaseComponent):
    ...
    ```python
    >> from kso_rag_core.pipelines.cot import Thought
    >> thought = Thought(
...
class ManualSequentialChainOfThought(BaseComponent):
    ...
    `kso_rag_core.llms.cot.Thought`. Please refer that section for
    ...
    **Create and run a chain of thought without "+" operator:**
    ...
    ```pycon
    >>> from kso_rag_core.pipelines.cot import Thought, ManualSequentialChainOfThought
    >>> llm = LCAzureChatOpenAI(...)
    >>> thought1 = Thought(
    >>>    prompt="Word {word} in {language} is ",
    >>>    post_process=lambda string: {"translated": string},
    ...
    **Create and run a chain of thought without "+" operator:** Please refer the
    `kso_rag_core.llms.cot.Thought` section for examples.
    ...
    llm: LLM = Param(help="The LLM model to use (base of kso_rag_core.llms.BaseLLM)")
