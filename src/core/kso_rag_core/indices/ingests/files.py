...
from kso_rag_core.base import BaseComponent, Document, Param
from kso_rag_core.indices.extractors import BaseDocParser
from kso_rag_core.indices.splitters import BaseSplitter, TokenSplitter
from kso_rag_core.loaders import (
    AdobeReader,
...
azure_reader = AzureAIDocumentIntelligenceLoader(
    ...
    cache_dir=getattr(flowsettings, "KSO_RAG_MARKDOWN_OUTPUT_DIR", None),
)
docling_reader = DoclingReader()
adobe_reader.vlm_endpoint = (
    azure_reader.vlm_endpoint
) = docling_reader.vlm_endpoint = getattr(flowsettings, "KSO_RAG_VLM_ENDPOINT", "")


KSO_RAG_DEFAULT_FILE_EXTRACTORS: dict[str, BaseReader] = {
...
class DocumentIngestor(BaseComponent):
    ...
    Args:
        ...
        override_file_extractors: override file extractors for specific file extensions
            The default file extractors are stored in `KSO_RAG_DEFAULT_FILE_EXTRACTORS`
    """
    ...
    def _get_reader(self, input_files: list[str | Path]):
        ...
        file_extractors: dict[str, BaseReader] = {
            ext: reader for ext, reader in KSO_RAG_DEFAULT_FILE_EXTRACTORS.items()
        }
