...
class CohereReranking(BaseReranking):
    ...
    use_key_from_kso_rag_ui: bool = False
    ...
    def run(self, documents: list[Document], query: str) -> list[Document]:
        ...
        if not self.cohere_api_key and self.use_key_from_kso_rag_ui:
            try:
                from kso_rag_ui.embeddings.manager import (
                    embedding_models_manager as embeddings,
                )

                cohere_model = embeddings.get("cohere")
                kso_rag_cohere_api_key = cohere_model._kwargs.get(  # type: ignore
                    "cohere_api_key"
                )
                if kso_rag_cohere_api_key != "your-key":
                    self.cohere_api_key = kso_rag_cohere_api_key
            except Exception as e:
                print("Cannot get Cohere API key from `kso_rag_ui`", e)
