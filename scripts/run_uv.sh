...
# kso-rag UV Installation Script
...
function install_dependencies() {
    ...
    print_success "Installing kso-rag with exact dependency resolution..."
    
    # Install in the exact same order as the original conda script
    uv pip install -e "src/core[all]"
    uv pip install -e "src/ui"
...
function setup_pdfjs() {
    ...
    local pdfjs_dir="src/ui/kso_rag_ui/kso_rag_ui/assets/prebuilt/pdfjs-4.0.379-dist"
...
function launch_app() {
    print_header "Launching kso-rag"
...
function main() {
    print_header "kso-rag UV-based Installation"
